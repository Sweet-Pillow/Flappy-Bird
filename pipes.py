...
class PipeTop(pg.sprite.Sprite):
    def __init__(self, posX, posY):
        ...
        self.image = pg.transform.flip(pg.image.load(
            self.pipe_top_image), False, True).convert()
...
class Pipes:
    def __init__(self, posX, num_pipe):
        ...
        self.pipe_bottom = PipeBottom(
            self.posX, self.posY + self.distanceY_between_pipes)

        self.score = pg.Surface((52, self.distanceY_between_pipes))
        self.rect_score = self.score.get_rect()
        self.rect_score.x = self.posX
        self.rect_score.y = self.posY
    ...
    def respawn(self):
        if self.pipe_top.rect.x <= -52:
            ...
            self.rect_score.x = self.distanceX_between_pipes * 2 - 52
            self.rect_score.y = self.posY
